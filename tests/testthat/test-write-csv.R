...
test_that("Check writing input for write.csv.summaryAlphaPart", {
  ...
  expect_error(write.csv.summaryAlphaPart(
    data.frame(x = 1:10),
    file = c("a", "b")
  ))
})
...
test_that("Check writing process for write.csv.summaryAlphaPart", {
  # Partition genetic values
  data(AlphaPart.ped)
  res <- AlphaPart(x = AlphaPart.ped, colPath = "country", colBV = c("trait1"))
  ...
  # Check content of files
  col <- c("generation", "N", "Sum", "1", "2")
  tmp <- read.csv2(file = retF[1])
  expect_equal(tmp, ret$trait1)
